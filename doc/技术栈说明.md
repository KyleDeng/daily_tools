...
**工具总览**：
- 💻 Coding工具（计算器、字符串处理、JSON校验、正则测试）
- 🌦️ 天气预报（48小时/15天）
- 📅 日历黄历（农历、节假日、黄历）
- 📰 新闻聚合（TrendRadar）
...
```
用户输入正则/修改flags/输入测试文本
    ↓
RegexTester组件 (useState: pattern, flags, testText)
    ↓
useEffect监听变化
    ↓
创建RegExp对象并执行匹配
    ↓
更新state (matches, error)
    ↓
渲染高亮文本和匹配列表
    ↓
（可选）执行替换操作
...
### 1. 程序员计算器
- **进制转换**：支持HEX/DEC/OCT/BIN四种进制实时转换，带空格格式化
- **位运算**：AND/OR/XOR/NOT/左移/右移完整支持
- **横向布局**：进制显示(400px) + 操作按钮并排，高效利用空间
- **智能禁用**：BIN模式禁用2-9按键，OCT模式禁用8-9按键
...
### 5. 正则表达式测试
- **实时匹配**：使用RegExp.exec()循环获取所有匹配项
- **高亮显示**：动态生成高亮标记，黄色背景区分匹配文本
- **捕获组支持**：展示所有捕获组内容，支持$1/$2等引用
- **10个示例**：预设邮箱、手机、URL等常用正则，一键加载
- **替换功能**：String.replace()配合捕获组实现复杂替换
